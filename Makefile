...
# PyTheory version into the manifests, and copies the LICENSE. The standalone
# repo is what gets pushed to GitHub and submitted to the plugin marketplace.
...
sync-skill:
	...
	@perl -pi -e 's/"version": "[^"]*"/"version": "$(VERSION)"/' \
		...
		"$(SKILL_REPO)/plugins/composing-with-pytheory/.claude-plugin/plugin.json"
	@echo "Synced skill v$(VERSION) -> $(SKILL_REPO)"
