...
parameters, some proven `recipes <Combining Effects_>`_, and how to
move any of it over time with `Automation`_.
...
Additionally, the `Per-Note Shaping`_ parameters (sub-oscillator,
noise layer, filter envelope, velocity-to-brightness, analog drift)
are applied per note, before the part chain.
...
The bus effects above process the part's whole signal. These
parameters work earlier — at the oscillator, on every note — and
they're what make a synth voice feel like an instrument: the filter
that goes "bwow," the breath under a flute, the weight under a bass.
